...
class StackWalkerInternal {
    public:
        ...
        BOOL Init(LPCSTR szSymPath) {
            ...
            if (::GetModuleFileName(NULL, tempStr, 4096) > 0) {
                strcat(tempStr, ".local");
                if (GetFileAttributes(tempStr) == INVALID_FILE_ATTRIBUTES) {
                    // ".local" file does not exist, so we can try to load the dbghelp.dll from the "Debugging Tools for Windows"
                    if (GetEnvironmentVariable("ProgramFiles", tempStr, 4096) > 0) {
                        strcat(tempStr, "\\Debugging Tools for Windows\\dbghelp.dll");
                        // now check if the file exists:
                        if (GetFileAttributes(tempStr) != INVALID_FILE_ATTRIBUTES) {
                            m_hDbhHelp = LoadLibrary(tempStr);
                        }
                    }
                    // Still not found? Then try to load the 64-Bit version:
                    if ((m_hDbhHelp == NULL) && (GetEnvironmentVariable("ProgramFiles", tempStr, 4096) > 0)) {
                        strcat(tempStr, "\\Debugging Tools for Windows 64-Bit\\dbghelp.dll");
                        if (GetFileAttributes(tempStr) != INVALID_FILE_ATTRIBUTES) {
                            m_hDbhHelp = LoadLibrary(tempStr);
                        }
                    }
...
#pragma pack( pop )
        ...
        BOOL GetModuleListTH32(HANDLE hProcess, DWORD pid) {
            ...
            // try both dlls...
            const TCHAR* dllname[] = { "kernel32.dll", "tlhelp32.dll" };
...
void StackWalker::OnCallstackEntry(CallstackEntryType eType, CallstackEntry& entry) {
    ...
    if ((eType != lastEntry) && (entry.offset != 0)) {
        ...
        if (entry.lineFileName[0] == 0) {
            ...
            _snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "%s (line %d): %s\n", entry.lineFileName, entry.lineNumber, entry.name);
        OnOutput(buffer);
    }
}
